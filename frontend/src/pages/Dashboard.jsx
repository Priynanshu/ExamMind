...
import { MessageSquare, TrendingUp, Flame, BookOpen, ArrowRight, Plus, Clock, Trash2 } from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    dispatch(fetchSessions({ limit: 5 }));
    dispatch(fetchProgress());
  }, [dispatch]);

  const levelLabel = { "class6-8": "Class 6-8", "class9-10": "Class 9-10", "class11-12": "Class 11-12", college: "College" };
  const streamLabel = { "science-pcm": "Science PCM", "science-pcb": "Science PCB", commerce: "Commerce", arts: "Arts", engineering: "Engineering", medical: "Medical", law: "Law", other: "General" };
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="text-sm mt-1 flex items-center gap-2" style={{ color: "var(--text-muted)" }}>
          <span className="px-2 py-0.5 rounded-full text-xs border" style={{ borderColor: "var(--border)" }}>{levelLabel[user?.profile?.educationLevel]}</span>
          <span className="px-2 py-0.5 rounded-full text-xs border" style={{ borderColor: "var(--border)" }}>{streamLabel[user?.profile?.stream]}</span>
          <span className="px-2 py-0.5 rounded-full text-xs border" style={{ borderColor: "var(--border)" }}>{user?.profile?.language === "hinglish" ? "🇮🇳 Hinglish" : "🇬🇧 English"}</span>
        </p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {stats.map(({ icon: Icon, label, value, color }, i) => (
          <motion.div key={label} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 + i * 0.05 }} className="card p-4">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="md:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/history" className="text-sm text-indigo-400 hover:text-indigo-300 flex items-center gap-1">View all <ArrowRight size={14} /></Link>
          </div>
          ...
          <div className="space-y-3">
            {sessionsLoading ? (
              ...
              <div className="card p-8 text-center">
                ...
                <p className="text-sm mb-4" style={{ color: "var(--text-muted)" }}>Ask your first doubt and start truly understanding your subjects.</p>
                <Link to="/session" className="btn-primary inline-flex items-center gap-2 text-sm px-4 py-2">Ask First Doubt <ArrowRight size={14} /></Link>
              </div>
            ) : (
              sessions.map((session) => (
                <motion.div key={session._id} whileHover={{ x: 2 }} className="card p-4">
                  <div className="flex items-start justify-between gap-2">
                    <Link to={`/session/${session._id}`} className="flex-1 min-w-0 block">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {session.isResolved && <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/10 text-green-400">✓ Resolved</span>}
                      </div>
                    </Link>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="space-y-4">
          <div>
            ...
            <div className="space-y-3">
              <Link to="/session" className="card p-4 flex items-center gap-3 hover:border-indigo-500/40 transition-all duration-200 block">
                <div className="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center flex-shrink-0"><Plus size={20} className="text-indigo-400" /></div>
                <div><p className="font-medium text-sm">New Doubt</p><p className="text-xs" style={{ color: "var(--text-muted)" }}>Start a new session</p></div>
              </Link>
              <Link to="/progress" className="card p-4 flex items-center gap-3 hover:border-indigo-500/40 transition-all duration-200 block">
                <div className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center flex-shrink-0"><TrendingUp size={20} className="text-purple-400" /></div>
                <div><p className="font-medium text-sm">My Progress</p><p className="text-xs" style={{ color: "var(--text-muted)" }}>View stats & badges</p></div>
              </Link>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {user.profile.subjects.map((subject) => (
                  <Link key={subject} to={`/session?subject=${subject}`}
                    ...
                    style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}>{subject}</Link>
                ))}
              </div>
            </div>
          )}
          ...
          {overview?.streak?.current > 0 && (
            <div className="card p-4 border-orange-500/20">
              <div className="flex items-center gap-2 mb-1"><Flame size={18} className="text-orange-400" /><span className="font-semibold text-sm">{overview.streak.current} Day Streak! 🔥</span></div>
              <p className="text-xs" style={{ color: "var(--text-muted)" }}>Keep it up! Come back tomorrow to maintain your streak.</p>
            </div>
          )}
        </motion.div>
      </div>
